...
export default function paginationField() {
  return {
    ...
    read(existing = [], { field, args, readField, cache }) {
      ...
      // Read the number of items, so we can make pagination. For some reason when deleting an item, this is null the first time and then runs two more times with the correct data?? ??!?!? ? ?? ? ? ? ? ?!?!?
