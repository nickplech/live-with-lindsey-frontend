...
const ALL_COMMUNITY_FAVORITES_QUERY = gql`
  query ALL_COMMUNITY_FAVORITES_QUERY {
    allVideoOnDemands {
      id
      name
      date
...
function CommunityFavoritesSlider({ category, user }) {
  const { data, error, loading } = useQuery(ALL_COMMUNITY_FAVORITES_QUERY)
  ...
  const listedOut = [...allVideoOnDemands].sort((a, b) => {
    return a._isFavoriteMeta.count > b._isFavoriteMeta.count ? -1 : 1
  })
      ...
      <Wrap>
        ...
        <div className="desktop-layout">
          <CarouselProvider
            ...
            totalSlides={allVideoOnDemands.length}
          >
            <ButtonBack disabled={allVideoOnDemands.length === 3}>
              &lsaquo;
            </ButtonBack>{' '}
...
export default CommunityFavoritesSlider
